...
window.onload = () => {
  ...
  app.assets.loadFromUrl("../../assets/models/X/model_purus_girl.glb", "model", (err, asset: pc.Asset | undefined) => {
    if (err) {
      console.error(err);
      return;
    }
    ...
    // load the texture
    app.assets.loadFromUrl("../../assets/textures/X/tex_purus_girl.jpg", "texture", (err, asset: pc.Asset | undefined) => {
      if (err) {
        console.error(err);
        return;
      }
